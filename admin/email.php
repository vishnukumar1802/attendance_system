...
// Get Admin Details for "From" field
$stmt = $pdo->prepare("SELECT username FROM admins WHERE id = ?");
$stmt->execute([$admin_id]);
$me = $stmt->fetch();
$from_display = "Admin: " . $me['username'];

// Fetch potential recipients (Admins + Employees)
$admins = $pdo->query("SELECT id, username FROM admins WHERE id != $admin_id")->fetchAll();
$employees = $pdo->query("SELECT id, first_name, last_name, employee_id FROM employees WHERE status='active' ORDER BY first_name ASC")->fetchAll();
